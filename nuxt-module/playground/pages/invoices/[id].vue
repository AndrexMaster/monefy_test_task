...
const updateInvoice = async () => {
  isSaving.value = true
  try {
    // Принудительно обновляем gross перед отправкой
...
<template>
  <div class="p-8 max-w-2xl mx-auto">
    <button @click="router.push('/invoices')" class="text-slate-400 hover:text-indigo-600 mb-6 flex items-center transition font-medium">
      ← Back to list
    </button>

    <div v-if="pending" class="py-20 text-center text-slate-400">Loading...</div>

    <div v-else class="bg-white p-8 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100">
      <div class="flex justify-between items-center mb-8">
        <h1 class="text-2xl font-bold text-slate-800">Invoice #{{ invoice.number }}</h1>
        <div v-if="invoice.status !== 'pending'" class="text-[10px] bg-slate-100 text-slate-500 px-3 py-1 rounded-lg uppercase font-bold">
          Read Only
        </div>
      </div>

      <div class="grid grid-cols-1 gap-5">
        <div v-for="(label, key) in labels" :key="key" class="flex flex-col gap-1.5 text-left">
          <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{{ label }}</label>

          <select v-if="key === 'status'" v-model="invoice[key]" class="input-style" :disabled="invoice.status !== 'pending'">
            <option value="pending">Pending</option>
            <option value="approved">Approved</option>
            <option value="rejected">Rejected</option>
          </select>

          <div v-else-if="key === 'gross_amount'" class="input-style bg-slate-50 text-indigo-600 font-bold border-dashed">
